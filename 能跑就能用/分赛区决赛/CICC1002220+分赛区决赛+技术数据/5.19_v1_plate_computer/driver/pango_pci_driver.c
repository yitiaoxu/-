...
struct PciPango pci_info = {
	._pango_pci_driver = {
		...
		._pci_io      = NULL,
		._pci_io_buff = NULL,
		._pci_status_bar = DMA_STATUS_BAR,
		._pci_status_io_size = 0,
		._pci_status_io = NULL,
		._pci_driver  = {
...
static int use_dma_status_poll = 1;
module_param(use_dma_status_poll, int, 0644);
MODULE_PARM_DESC(use_dma_status_poll,
		 "Poll BAR0+0x130 for DMA completion (requires updated FPGA bitstream)");
...
static unsigned int dma_row_timeout_us(unsigned int row_bytes)
{
	unsigned int us = 1000;

	if (row_bytes && row_bytes < PCIE_ROW_BYTES)
		us = 1000 * row_bytes / PCIE_ROW_BYTES;
	if (us < 500)
		us = 500;
	return us;
}

static void wait_dma_row_ready(unsigned int row_bytes)
{
	volatile int i;
	unsigned int loops = 8000;

	if (row_bytes && row_bytes < 3840)
		loops = 8000 * row_bytes / 3840;
	if (loops < 2000)
		loops = 2000;
...
static int wait_dma_done(struct PciPango *pci_pango, unsigned int row_bytes)
{
	void __iomem *status_io = pci_pango->_pango_pci_driver._pci_status_io;
	unsigned int timeout_us = dma_row_timeout_us(row_bytes);
	ktime_t deadline;

	if (!use_dma_status_poll || !status_io) {
		wait_dma_row_ready(row_bytes);
		return 0;
	}

	deadline = ktime_add_us(ktime_get(), timeout_us);
	while (ktime_before(ktime_get(), deadline)) {
		u32 st = ioread32(status_io + DMA_STATUS_OFFSET);

		if (st & DMA_STATUS_4KB_ERR)
			return -EIO;
	...
	printk(KERN_WARNING "pango_pci: DMA row timeout (%u us, row_bytes=%u)\n",
	       timeout_us, row_bytes);
	return -ETIMEDOUT;
}
...
static enum dma_map_mode dma_map_mode = DMA_MAP_NONE;
static unsigned int dma_map_current_len = 0;
static unsigned int dma_row_len_dwords = 0;
static unsigned int dma_frame_height = 0;
static unsigned int dma_frame_row_bytes = 0;
static void *dma_frame_assembly_buf = NULL;
static dma_addr_t dma_frame_assembly_bus_addr = 0;
static size_t dma_frame_assembly_size = 0;
static void *dma_infer_assembly_buf = NULL;
static dma_addr_t dma_infer_assembly_bus_addr = 0;
static size_t dma_infer_assembly_size = 0;
static int dma_infer_map_ready = 0;

static void set_dma_src_sel(struct PciPango *pci_pango, unsigned int sel)
{
	iowrite32(sel & 0x1u, pci_pango->_pango_pci_driver._pci_io + DMA_SRC_SEL_OFFSET);
}

static int read_plate_roi(struct PciPango *pci_pango, PLATE_ROI_INFO *roi)
{
	void __iomem *io = pci_pango->_pango_pci_driver._pci_io;
	u32 status;

	if (!io || !roi)
		return -EINVAL;

	roi->x = (u16)(ioread32(io + PLATE_ROI_X_OFFSET) & 0xFFFu);
	roi->y = (u16)(ioread32(io + PLATE_ROI_Y_OFFSET) & 0xFFFu);
	roi->w = (u16)(ioread32(io + PLATE_ROI_W_OFFSET) & 0xFFFu);
	roi->h = (u16)(ioread32(io + PLATE_ROI_H_OFFSET) & 0xFFFu);
	status = ioread32(io + PLATE_ROI_STATUS_OFFSET);
	roi->valid = (u8)(status & 0x1u);
	roi->frame_id = (u8)((status >> 8) & 0xFFu);
	roi->reserved = 0;
	return 0;
}

static long ioctl_get_plate_roi(struct PciPango *pci_pango, unsigned long arg)
{
	PLATE_ROI_INFO roi;
	int ret;

	ret = read_plate_roi(pci_pango, &roi);
	if (ret)
		return ret;

	if (copy_to_user((PLATE_ROI_INFO __user *)arg, &roi, sizeof(PLATE_ROI_INFO)))
		return -EFAULT;
	return 0;
}

static int dma_trigger_row_read(struct PciPango *pci_pango, dma_addr_t row_addr,
				unsigned int row_bytes)
{
	unsigned int row_len = dma_row_len_dwords ? dma_row_len_dwords : dma_map_current_len;
	...
	dma_info.addr_w.addr = row_addr;
	dma_info.cmd.data.op_type = 1;
	dma_info.cmd.data.length = row_len - 1;
	...
	return wait_dma_done(pci_pango, row_bytes);
}

static int capture_frame_to_assembly(struct PciPango *pci_pango,
				     unsigned int height,
				     unsigned int row_bytes)
{
	unsigned int row;
	...
	if (!dma_info.addr_w.data_buf || !dma_frame_assembly_buf)
		return -EINVAL;

	for (row = 0; row < height; row++) {
		spin_lock(&dma_info.addr_r.lock);
		ret = dma_trigger_row_read(pci_pango,
			dma_frame_assembly_bus_addr +
				(dma_addr_t)row * PCIE_ROW_STRIDE,
			row_bytes);
...
static int copy_frame_compact_to_user(void __user *dst,
				      unsigned int height,
				      unsigned int row_bytes)
{
	unsigned int row;
	...
	for (row = 0; row < height; row++) {
		if (copy_to_user(dst + (size_t)row * row_bytes,
				 (char *)dma_frame_assembly_buf +
				 (size_t)row * PCIE_ROW_STRIDE,
				 row_bytes))
			return -EFAULT;
	}
	return 0;
...
static int capture_infer_frame_to_assembly(struct PciPango *pci_pango,
					   unsigned int height,
					   unsigned int row_bytes)
{
	unsigned int row;
	unsigned int saved_row_len = dma_row_len_dwords;
	unsigned int infer_row_len = row_bytes / 4;
	int ret;

	if (!dma_info.addr_w.data_buf || !dma_infer_assembly_buf || !infer_row_len)
		return -EINVAL;

	dma_row_len_dwords = infer_row_len;
	set_dma_src_sel(pci_pango, 1);

	for (row = 0; row < height; row++) {
		spin_lock(&dma_info.addr_r.lock);
		ret = dma_trigger_row_read(pci_pango,
			dma_infer_assembly_bus_addr +
				(dma_addr_t)row * PCIE_RGB888_ROW_STRIDE,
			row_bytes);
		spin_unlock(&dma_info.addr_r.lock);
		if (ret) {
			set_dma_src_sel(pci_pango, 0);
			dma_row_len_dwords = saved_row_len;
			return ret;
		}
	}
...
static int copy_infer_frame_compact_to_user(void __user *dst,
					    unsigned int height,
					    unsigned int row_bytes)
{
	unsigned int row;
	...
	for (row = 0; row < height; row++) {
		if (copy_to_user(dst + (size_t)row * row_bytes,
				 (char *)dma_infer_assembly_buf +
				 (size_t)row * PCIE_RGB888_ROW_STRIDE,
				 row_bytes))
			return -EFAULT;
	}
	return 0;
...
static long ioctl_map_infer_frame(struct PciPango *pci_pango, unsigned long arg)
{
	FRAME_CAPTURE frame_cap;
	unsigned int height;
	unsigned int row_bytes;
	size_t row_map_size;
	size_t frame_map_size;

	if (copy_from_user(&frame_cap, (FRAME_CAPTURE __user *)arg,
			   sizeof(FRAME_CAPTURE)))
		return -EFAULT;
	if (!op_dev)
		return -ENODEV;

	height = frame_cap.height ? frame_cap.height : PCIE_FRAME_HEIGHT;
	row_bytes = frame_cap.row_bytes ? frame_cap.row_bytes : PCIE_RGB888_ROW_BYTES;
	if (height != PCIE_FRAME_HEIGHT || row_bytes != PCIE_RGB888_ROW_BYTES)
		return -EINVAL;

	row_map_size = PAGE_ALIGN(row_bytes);
	frame_map_size = PAGE_ALIGN((size_t)PCIE_RGB888_ROW_STRIDE * height);

	spin_lock(&dma_info.addr_r.lock);

	if (dma_infer_assembly_buf && dma_infer_assembly_size == frame_map_size) {
		dma_infer_map_ready = 1;
		spin_unlock(&dma_info.addr_r.lock);
		return 0;
	}

	if (dma_infer_assembly_buf) {
		dma_free_coherent(&op_dev->dev, dma_infer_assembly_size,
			dma_infer_assembly_buf, dma_infer_assembly_bus_addr);
		dma_infer_assembly_buf = NULL;
		dma_infer_assembly_bus_addr = 0;
		dma_infer_assembly_size = 0;
	}
	...
	if (!dma_info.addr_w.data_buf) {
		...
		if (!dma_info.addr_r.data_buf || !dma_info.addr_w.data_buf) {
			if (dma_info.addr_r.data_buf) {
				dma_free_coherent(&op_dev->dev, row_map_size,
		...
		dma_info.addr_r.buf_size = row_map_size;
		dma_info.addr_w.buf_size = row_map_size;
		dma_info.addr_r.addr = dma_info.addr_r.base_addr;
		dma_info.addr_w.addr = dma_info.addr_w.base_addr;
		dma_info.addr_r.addr_size = ((dma_info.addr_r.addr >> 32) > 0) ? 1 : 0;
		dma_info.addr_w.addr_size = ((dma_info.addr_w.addr >> 32) > 0) ? 1 : 0;
	}

	dma_infer_assembly_buf = dma_alloc_coherent(&op_dev->dev,
				frame_map_size,
				&dma_infer_assembly_bus_addr,
				GFP_KERNEL);
	if (!dma_infer_assembly_buf) {
	...
	dma_infer_assembly_size = frame_map_size;
	dma_infer_map_ready = 1;
	spin_unlock(&dma_info.addr_r.lock);
	return 0;
}

static long ioctl_read_infer_frame(struct PciPango *pci_pango, unsigned long arg)
{
	FRAME_CAPTURE frame_cap;
	unsigned int height;
	unsigned int row_bytes;
	...
	if (!dma_infer_map_ready || !dma_infer_assembly_buf || !dma_info.addr_w.data_buf)
		return -EINVAL;
	...
	ret = capture_infer_frame_to_assembly(pci_pango, height, row_bytes);
	if (ret)
		return ret;

	if (frame_cap.user_buf) {
		if (copy_infer_frame_compact_to_user((void __user *)frame_cap.user_buf,
						     height, row_bytes))
			return -EFAULT;
	}
	return 0;
...
static long ioctl_read_frame(struct PciPango *pci_pango, unsigned long arg)
{
	FRAME_CAPTURE frame_cap;
	unsigned int height;
	unsigned int row_bytes;
	...
	if (dma_map_mode != DMA_MAP_FRAME || !dma_info.addr_w.data_buf ||
	    !dma_frame_assembly_buf)
		return -EINVAL;
	...
	height = frame_cap.height ? frame_cap.height : dma_frame_height;
	row_bytes = frame_cap.row_bytes ? frame_cap.row_bytes : dma_frame_row_bytes;
	if (height != dma_frame_height || row_bytes != dma_frame_row_bytes)
		return -EINVAL;

	ret = capture_frame_to_assembly(pci_pango, height, row_bytes);
	if (ret)
		return ret;

	if (frame_cap.user_buf) {
		if (copy_frame_compact_to_user((void __user *)frame_cap.user_buf,
					       height, row_bytes))
			return -EFAULT;
	}
	return 0;
...
static void free_dma_addr_pair(void)
{
	if (!op_dev)
		return;

	if (dma_info.addr_r.data_buf) {
		dma_free_coherent(&op_dev->dev, dma_info.addr_r.buf_size,
			dma_info.addr_r.data_buf, dma_info.addr_r.base_addr);
		dma_info.addr_r.data_buf = NULL;
		dma_info.addr_r.base_addr = 0;
		dma_info.addr_r.buf_size = 0;
	}
	if (dma_info.addr_w.data_buf) {
		dma_free_coherent(&op_dev->dev, dma_info.addr_w.buf_size,
			dma_info.addr_w.data_buf, dma_info.addr_w.base_addr);
		dma_info.addr_w.data_buf = NULL;
		dma_info.addr_w.base_addr = 0;
		dma_info.addr_w.buf_size = 0;
	}
	if (dma_frame_assembly_buf) {
		dma_free_coherent(&op_dev->dev, dma_frame_assembly_size,
			dma_frame_assembly_buf, dma_frame_assembly_bus_addr);
		dma_frame_assembly_buf = NULL;
		dma_frame_assembly_bus_addr = 0;
		dma_frame_assembly_size = 0;
	}
	if (dma_infer_assembly_buf) {
		dma_free_coherent(&op_dev->dev, dma_infer_assembly_size,
			dma_infer_assembly_buf, dma_infer_assembly_bus_addr);
		...
		dma_infer_assembly_bus_addr = 0;
		dma_infer_assembly_size = 0;
		dma_infer_map_ready = 0;
	}
	dma_map_current_len = 0;
	dma_row_len_dwords = 0;
	dma_frame_height = 0;
	dma_frame_row_bytes = 0;
	dma_map_mode = DMA_MAP_NONE;
}
...
long pango_cdev_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	if (cmd == PCI_READ_FRAME_CMD)
		return ioctl_read_frame(pci_pango, arg);

	if (cmd == PCI_READ_INFER_FRAME_CMD)
		return ioctl_read_infer_frame(pci_pango, arg);

	if (cmd == PCI_GET_PLATE_ROI_CMD)
		return ioctl_get_plate_roi(pci_pango, arg);

	if (cmd == PCI_MAP_INFER_FRAME_CMD) {
		long ret = ioctl_map_infer_frame(pci_pango, arg);
		return ret;
	}
	...
	switch(cmd)
	{
		...
		case PCI_MAP_ADDR_CMD:
			...
			if (dma_info.addr_w.data_buf &&
			    dma_map_current_len == dma_operation.current_len) {
				spin_unlock(&dma_info.addr_r.lock);
				break;
			}
			if (dma_info.addr_w.data_buf) {
			...
			{
				size_t map_size = PAGE_ALIGN(dma_operation.current_len * 4);
				dma_info.addr_r.data_buf = dma_alloc_coherent(&op_dev->dev,
						 map_size,
						 &dma_info.addr_r.base_addr,
						 GFP_KERNEL);
			...
			dma_map_current_len = dma_operation.current_len;
			dma_row_len_dwords = dma_operation.current_len;
			dma_info.addr_r.buf_size = PAGE_ALIGN(dma_operation.current_len * 4);
			dma_info.addr_w.buf_size = PAGE_ALIGN(dma_operation.current_len * 4);
			dma_info.addr_r.addr = dma_info.addr_r.base_addr;
			dma_info.addr_w.addr = dma_info.addr_w.base_addr;
			dma_info.addr_r.addr_size = ((dma_info.addr_r.addr >> 32) > 0) ? 1 : 0;
			dma_info.addr_w.addr_size = ((dma_info.addr_w.addr >> 32) > 0) ? 1 : 0;
			...
			dma_map_mode = DMA_MAP_ROW;
			spin_unlock(&dma_info.addr_r.lock);
		break;

		case PCI_MAP_FRAME_CMD:
			spin_lock(&dma_info.addr_r.lock);
			{
				FRAME_CAPTURE frame_cap;
				unsigned int height;
				...
				height = frame_cap.height ? frame_cap.height : PCIE_FRAME_HEIGHT;
				row_bytes = frame_cap.row_bytes ? frame_cap.row_bytes : PCIE_ROW_BYTES;
				row_map_size = PAGE_ALIGN(row_bytes);
				frame_map_size = PAGE_ALIGN((size_t)PCIE_ROW_STRIDE * height);

				if (dma_info.addr_w.data_buf &&
				    dma_frame_assembly_buf &&
				    dma_map_mode == DMA_MAP_FRAME &&
				    dma_info.addr_w.buf_size == row_map_size &&
				    dma_frame_assembly_size == frame_map_size &&
				    dma_frame_height == height &&
				    dma_frame_row_bytes == row_bytes) {
					spin_unlock(&dma_info.addr_r.lock);
					break;
				}
				...
				dma_info.addr_w.data_buf = dma_alloc_coherent(&op_dev->dev,
						 ...
						 &dma_info.addr_w.base_addr,
						 GFP_KERNEL);
				dma_frame_assembly_buf = dma_alloc_coherent(&op_dev->dev,
						 frame_map_size,
						 &dma_frame_assembly_bus_addr,
						 GFP_KERNEL);
				if (!dma_info.addr_r.data_buf || !dma_info.addr_w.data_buf ||
				...
				dma_row_len_dwords = row_bytes / 4;
				dma_frame_height = height;
				dma_frame_row_bytes = row_bytes;
				dma_map_current_len = dma_row_len_dwords;
				dma_info.addr_r.buf_size = row_map_size;
				dma_info.addr_w.buf_size = row_map_size;
				dma_frame_assembly_size = frame_map_size;
				dma_info.addr_r.addr = dma_info.addr_r.base_addr;
				dma_info.addr_w.addr = dma_info.addr_w.base_addr;
				dma_info.addr_r.addr_size =
					((dma_info.addr_r.addr >> 32) > 0) ? 1 : 0;
				dma_info.addr_w.addr_size =
					((dma_info.addr_w.addr >> 32) > 0) ? 1 : 0;
				dma_info.cmd.data.length = dma_row_len_dwords - 1;
				dma_info.cmd.data.addr_type = dma_info.addr_r.addr_size;
				dma_map_mode = DMA_MAP_FRAME;
			}
			spin_unlock(&dma_info.addr_r.lock);
		...
		case PCI_DMA_WRITE_CMD:												/* CPU读数据 */
			...
			{
				int dma_ret = dma_trigger_row_read(pci_pango, dma_info.addr_w.addr,
					     dma_map_current_len * 4);
				spin_unlock(&dma_info.addr_r.lock);
				if (dma_ret) {
					up(&pci_pango->_sem);
					return dma_ret;
		...
		case PCI_READ_DMA_DATA_CMD:											/* 仅拷贝 DMA 行数据 */
			spin_lock(&dma_info.addr_r.lock);
			if (!dma_info.addr_w.data_buf) {
				spin_unlock(&dma_info.addr_r.lock);
				return -EINVAL;
			}
			if (copy_to_user((void __user *)arg, dma_info.addr_w.data_buf,
				 dma_map_current_len * 4)) {
				spin_unlock(&dma_info.addr_r.lock);
				return -EFAULT;
			}
		...
		case PCI_PERFORMANCE_START_CMD:
			...
			performance_config.addr.buf_size = DMA_MAX_PACKET_SIZE * 10 * 2;
		...
		case PCI_PERFORMANCE_END_CMD:
			...
			dma_free_coherent(&op_dev->dev, performance_config.addr.buf_size,
				performance_config.addr.data_buf, performance_config.addr.base_addr);
			performance_config.addr.data_buf = NULL;
			performance_config.addr.base_addr = 0;
			performance_config.addr.buf_size = 0;
			spin_unlock(&performance_config.addr.lock);
		break;
	}
...
{
	if (!op_dev || !pci_info._pango_pci_driver._pci_io)
		return -ENODEV;
...
int pango_cdev_mmap(struct file *filp, struct vm_area_struct *vma)
{
	...
	if (dma_map_mode == DMA_MAP_FRAME && dma_frame_assembly_buf) {
		mmap_buf = dma_frame_assembly_buf;
		mmap_size = dma_frame_assembly_size;
	} else if (dma_info.addr_w.data_buf) {
		mmap_buf = dma_info.addr_w.data_buf;
		mmap_size = dma_info.addr_w.buf_size;
	} else {
		return -EINVAL;
	}
	...
	ret = dma_mmap_coherent(&op_dev->dev, vma, mmap_buf,
				dma_map_mode == DMA_MAP_FRAME ?
					dma_frame_assembly_bus_addr :
					dma_info.addr_w.base_addr,
				mmap_size);
	...
	pfn = page_to_pfn(virt_to_page(mmap_buf));
	vma->vm_page_prot = pgprot_writecombine(vma->vm_page_prot);
	ret = remap_pfn_range(vma, vma->vm_start, pfn, size, vma->vm_page_prot);
	if (ret)
		printk(KERN_ERR "pango_pci: mmap fallback remap_pfn_range failed: %d\n", ret);
	return ret;
}
...
int pci_driver_probe(struct pci_dev *dev, const struct pci_device_id *device_id)
{
	...
	unsigned long status_bar_address = 0;
	...
	if (pci_resource_len(dev, pci_info._pango_pci_driver._pci_status_bar) > 0) {
		result = pci_request_region(dev, pci_info._pango_pci_driver._pci_status_bar,
					    NULL);
		if (result == 0) {
			pci_info._pango_pci_driver._pci_status_io_size =
				pci_resource_len(dev, pci_info._pango_pci_driver._pci_status_bar);
			status_bar_address = pci_resource_start(dev,
					pci_info._pango_pci_driver._pci_status_bar);
			pci_info._pango_pci_driver._pci_status_io =
				ioremap(status_bar_address,
					pci_info._pango_pci_driver._pci_status_io_size);
			if (!pci_info._pango_pci_driver._pci_status_io) {
				pci_release_region(dev,
					pci_info._pango_pci_driver._pci_status_bar);
				pci_info._pango_pci_driver._pci_status_io_size = 0;
			}
		} else {
			printk(KERN_WARNING "pango_pci: BAR%d request failed (%d), "
			       "falling back to timed wait\n",
			       pci_info._pango_pci_driver._pci_status_bar, result);
			result = 0;
		}
	}
...
void pci_driver_remove(struct pci_dev *dev)
{
	...
	if (pci_info._pango_pci_driver._pci_status_io) {
		iounmap(pci_info._pango_pci_driver._pci_status_io);
		pci_release_region(dev, pci_info._pango_pci_driver._pci_status_bar);
	}
    LOG("iounmap\n");
	...
	if (op_dev == dev)
		op_dev = NULL;
	pci_info._pango_pci_driver._pci_io = NULL;
	pci_info._pango_pci_driver._pci_io_size = 0;
	pci_info._pango_pci_driver._pci_status_io = NULL;
	pci_info._pango_pci_driver._pci_status_io_size = 0;
...
int init_pango_cdev_class(void)
{
	...
	device_create(pci_info._cdev_class, NULL, pci_dev_info._dev, NULL,
		      "%s", pci_dev_info._dev_name);
	return 0;
}

void exit_pango_cdev_class(void)
{
	if (!pci_info._cdev_class)
		return;
