...
#define PCI_READ_DMA_DATA_CMD    		_IOWR(TYPE, 10, int)							/* 仅拷贝 DMA 行数据到用户缓冲 */
#define PCI_MAP_FRAME_CMD    			_IOWR(TYPE, 11, int)							/* 映射整帧 DMA 缓冲 */
#define PCI_READ_FRAME_CMD    			_IOWR(TYPE, 12, int)							/* 内核批处理整帧采集 */
#define PCI_MAP_INFER_FRAME_CMD			_IOWR(TYPE, 13, int)							/* 映射推理 RGB888 整帧缓冲 */
#define PCI_READ_INFER_FRAME_CMD		_IOWR(TYPE, 14, int)							/* 推理 RGB888 整帧采集 */
#define PCI_GET_PLATE_ROI_CMD			_IOWR(TYPE, 15, int)							/* 读取 FPGA 粗定位 ROI */
...
#define DMA_SRC_SEL_OFFSET				0x1C0										/* 写: 0=RGB565预览 1=RGB888推理(P1-B FPGA) */
#define PLATE_ROI_X_OFFSET				0x1D0
#define PLATE_ROI_Y_OFFSET				0x1E0
#define PLATE_ROI_W_OFFSET				0x1F0
#define PLATE_ROI_H_OFFSET				0x200
#define PLATE_ROI_STATUS_OFFSET			0x210
#define PLATE_DEBUG_STAGE_OFFSET		0x220
#define PLATE_GAMMA_IDX_OFFSET			0x230
#define PLATE_CONTRAST_LO_OFFSET		0x240
#define PLATE_CONTRAST_HI_OFFSET		0x250
#define PLATE_SOBEL_THRESH_OFFSET		0x260

#define MAX_BLOCK_SIZE   				1024										/* 位流分块缓存区最大值 */
#define DMA_STATUS_BAR					0											/* BAR0: Host MRd 读 DMA 状态 */
...
#define DMA_STATUS_OFFSET				0x130										/* BAR0: dma_done/busy/4kb_err */
#define DMA_STATUS_DONE					(1u << 0)
#define DMA_STATUS_BUSY					(1u << 1)
#define DMA_STATUS_4KB_ERR				(1u << 2)
...
struct PangoPciDriver
{
	...
	void             *_pci_io_buff;
	int               _pci_status_bar;
	unsigned long     _pci_status_io_size;
	void __iomem     *_pci_status_io;
