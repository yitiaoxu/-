...
begin
    if(!rst_n)
        ...
        case(i_apb_paddr)
            ...
            9'h130: o_apb_prdata <= o_dma_status;
...
// Host-visible DMA status (BAR0 byte offset 0x130 via MRd completion mux)
reg dma_status_done;
reg dma_status_busy;
reg dma_status_4kb_err;

always@(posedge clk or negedge rst_n)
begin
    if(!rst_n) begin
        dma_status_done    <= 1'b0;
        dma_status_busy    <= 1'b0;
        dma_status_4kb_err <= 1'b0;
    end else if(device_ep) begin
        if(dma_cmd_reg_vld && dma_wr_rd_cmd_flag) begin
            dma_status_done    <= 1'b0;
            dma_status_4kb_err <= 1'b0;
        end
        if(dma_cmd_l_addr_vld && dma_wr_rd_cmd_flag)
            dma_status_busy <= 1'b1;
        if(cross_4kb_boundary) begin
            dma_status_4kb_err <= 1'b1;
            dma_status_busy    <= 1'b0;
        end
        if(ack_rcv) begin
            dma_status_done <= 1'b1;
            dma_status_busy <= 1'b0;
        end
    end
end

assign o_dma_status = {29'b0, dma_status_4kb_err, dma_status_busy, dma_status_done};
