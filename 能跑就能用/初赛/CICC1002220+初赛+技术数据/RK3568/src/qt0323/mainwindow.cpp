...
static bool pcie_read_one_frame_pango(int fd, std::vector<uint8_t>& frame, DMA_OPERATION& dma_op) {
    ...
    // 1) 先读帧首 224B metadata
    dma_op.offset_addr = 0;
    dma_op.current_len = kDmaTailDw;
    memset(dma_op.data.write_buf, 0, DMA_MAX_PACKET_SIZE);
    memset(dma_op.data.read_buf, 0, DMA_MAX_PACKET_SIZE);
    if (ioctl(fd, PCI_MAP_ADDR_CMD, &dma_op) < 0)
        return false;
    memset(dma_op.data.read_buf, 0, DMA_MAX_PACKET_SIZE);
    if (ioctl(fd, PCI_DMA_WRITE_CMD, &dma_op) < 0) {
        ioctl(fd, PCI_UMAP_ADDR_CMD, &dma_op);
        return false;
    }
    for (volatile int k = 0; k < 2500; ++k) { }
    if (ioctl(fd, PCI_READ_FROM_KERNEL_CMD, &dma_op) < 0) {
        ioctl(fd, PCI_UMAP_ADDR_CMD, &dma_op);
        return false;
    }
    memcpy(frame.data() + off, dma_op.data.read_buf, kPcieMetaBytes);
    off += kPcieMetaBytes;
    if (ioctl(fd, PCI_UMAP_ADDR_CMD, &dma_op) < 0)
        return false;

    // 2) 再读 720 行，每行 2560B RGB565
    ...
    for (int row = 0; row < kPcieImgH; ++row) {
        memset(dma_op.data.read_buf, 0, DMA_MAX_PACKET_SIZE);
        if (ioctl(fd, PCI_DMA_WRITE_CMD, &dma_op) < 0) {
            ioctl(fd, PCI_UMAP_ADDR_CMD, &dma_op);
            return false;
        ...
        memcpy(frame.data() + off, dma_op.data.read_buf, kDmaChunkBytes);
        off += kDmaChunkBytes;
    }
    if (ioctl(fd, PCI_UMAP_ADDR_CMD, &dma_op) < 0)
        return false;
