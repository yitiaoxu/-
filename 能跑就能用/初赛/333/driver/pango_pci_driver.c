...
long pango_cdev_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	switch(cmd)
	{
		...
		case PCI_MAP_ADDR_CMD:
			...
			request_bytes = (size_t)dma_operation.current_len * 4;
			if (dma_operation.current_len == 0 || request_bytes > DMA_MAX_PACKET_SIZE) {
				printk(KERN_ALERT "PCI_MAP_ADDR_CMD invalid len: %u dw (%zu bytes)\n",
				       dma_operation.current_len, request_bytes);
				spin_unlock(&dma_info.addr_r.lock);
				return -EINVAL;
			}
			...
			if (!dma_info.addr_r.data_buf || !dma_info.addr_w.data_buf) {
				if (dma_info.addr_r.data_buf) {
					dma_free_coherent(&op_dev->dev, request_bytes, dma_info.addr_r.data_buf, dma_info.addr_r.addr);
					dma_info.addr_r.data_buf = NULL;
				}
				if (dma_info.addr_w.data_buf) {
					dma_free_coherent(&op_dev->dev, request_bytes, dma_info.addr_w.data_buf, dma_info.addr_w.addr);
					dma_info.addr_w.data_buf = NULL;
				}
				spin_unlock(&dma_info.addr_r.lock);
				printk(KERN_ALERT "PCI_MAP_ADDR_CMD dma_alloc_coherent failed, bytes=%zu\n", request_bytes);
				return -ENOMEM;
			}
			dma_map_bytes = request_bytes;
		...
		case PCI_WRITE_TO_KERNEL_CMD:										
			...
			request_bytes = (size_t)dma_operation.current_len * 4;
			if (dma_operation.current_len == 0 || request_bytes > DMA_MAX_PACKET_SIZE || request_bytes > dma_map_bytes) {
				spin_unlock(&dma_info.addr_r.lock);
				printk(KERN_ALERT "PCI_WRITE_TO_KERNEL_CMD invalid len: %u dw (%zu bytes), map=%zu\n",
				       dma_operation.current_len, request_bytes, dma_map_bytes);
				return -EINVAL;
			}
			memcpy(dma_info.addr_r.data_buf, dma_operation.data.write_buf, request_bytes);
			memset(dma_info.addr_w.data_buf, 0, request_bytes);
			spin_unlock(&dma_info.addr_r.lock);
		break;
		...
		case PCI_DMA_WRITE_CMD:												/* CPU读数据 */
			...
			if (dma_operation.current_len == 0 || request_bytes > DMA_MAX_PACKET_SIZE || request_bytes > dma_map_bytes) {
				spin_unlock(&dma_info.addr_r.lock);
				printk(KERN_ALERT "PCI_DMA_WRITE_CMD invalid len: %u dw (%zu bytes), map=%zu\n",
				       dma_operation.current_len, request_bytes, dma_map_bytes);
				return -EINVAL;
			}
			dma_info.cmd.data.length = dma_operation.current_len - 1;
		...
		case PCI_READ_FROM_KERNEL_CMD:										/* 将数据写入CPU缓存 */
			...
			if (dma_operation.current_len == 0 || request_bytes > DMA_MAX_PACKET_SIZE || request_bytes > dma_map_bytes) {
				spin_unlock(&dma_info.addr_r.lock);
				printk(KERN_ALERT "PCI_READ_FROM_KERNEL_CMD invalid len: %u dw (%zu bytes), map=%zu\n",
				       dma_operation.current_len, request_bytes, dma_map_bytes);
				return -EINVAL;
			}
			memcpy(dma_operation.data.read_buf, dma_info.addr_w.data_buf, request_bytes);
		...
		case PCI_UMAP_ADDR_CMD:
			...
			if (dma_info.addr_r.data_buf)
				dma_free_coherent(&op_dev->dev, dma_map_bytes, dma_info.addr_r.data_buf, dma_info.addr_r.addr);
    		if (dma_info.addr_w.data_buf)
				dma_free_coherent(&op_dev->dev, dma_map_bytes, dma_info.addr_w.data_buf, dma_info.addr_w.addr);
			dma_info.addr_r.data_buf = NULL;
			dma_info.addr_w.data_buf = NULL;
			dma_map_bytes = 0;
			spin_unlock(&dma_info.addr_r.lock);
