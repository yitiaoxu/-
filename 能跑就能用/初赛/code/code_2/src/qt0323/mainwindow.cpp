...
void InferenceThread::processPCIeStream() {
    ...
    constexpr uint32_t PCIE_MAGIC = 0x524B3031;
    constexpr int width = 1280;
    constexpr int height = 720;
    constexpr int PCIE_META_LEN = 224;
    constexpr int PCIE_FRAME_BYTES = PCIE_META_LEN + width * height * 2;
    constexpr int DMA_CHUNK_BYTES = 2560;
    constexpr size_t STREAM_KEEP_TAIL = static_cast<size_t>(PCIE_FRAME_BYTES) * 2;

    struct PcieFrameMeta {
        uint32_t magic;
        uint32_t frame_id;
        uint32_t width;
        uint32_t height;
    ...
    stream_buffer.reserve(static_cast<size_t>(PCIE_FRAME_BYTES) * 3);

    DMA_OPERATION dma_operation{};
    dma_operation.current_len = DMA_CHUNK_BYTES / 4;
    ...
    if (ioctl(fd, PCI_MAP_ADDR_CMD, &dma_operation) < 0) {
        std::cerr << "[PCIe] PCI_MAP_ADDR_CMD failed, errno="
                  << errno << " (" << std::strerror(errno) << ")" << std::endl;
        close(fd);
        return;
    }
    ...
    while (keep_running) {
        if (job_cnt - result_cnt < numThreads) {
            ...
            if (ioctl(fd, PCI_DMA_WRITE_CMD, &dma_operation) < 0) {
                std::cerr << "[PCIe] PCI_DMA_WRITE_CMD failed, errno="
                          << errno << " (" << std::strerror(errno) << ")" << std::endl;
                usleep(1000);
                continue;
            }
            for (volatile int k = 0; k < 2500; k++);
            if (ioctl(fd, PCI_READ_FROM_KERNEL_CMD, &dma_operation) < 0) {
                std::cerr << "[PCIe] PCI_READ_FROM_KERNEL_CMD failed, errno="
                          << errno << " (" << std::strerror(errno) << ")" << std::endl;
                usleep(1000);
                continue;
            }
            ...
            stream_buffer.insert(stream_buffer.end(),
                                 dma_operation.data.read_buf,
                                 dma_operation.data.read_buf + DMA_CHUNK_BYTES);

            while (stream_buffer.size() >= static_cast<size_t>(PCIE_FRAME_BYTES)) {
                size_t magic_pos = stream_buffer.size();
                const size_t max_search = stream_buffer.size() - sizeof(uint32_t) + 1;
                for (size_t i = 0; i < max_search; i += sizeof(uint32_t)) {
                    if (read_u32(stream_buffer.data() + i) == PCIE_MAGIC) {
                        magic_pos = i;
                        break;
                    }
                }

                if (magic_pos == stream_buffer.size()) {
                    if (stream_buffer.size() > STREAM_KEEP_TAIL) {
                        stream_buffer.erase(stream_buffer.begin(),
                                            stream_buffer.end() - STREAM_KEEP_TAIL);
                    }
                    break;
                }
                ...
                if (magic_pos > 0) {
                    stream_buffer.erase(stream_buffer.begin(),
                                        stream_buffer.begin() + static_cast<std::ptrdiff_t>(magic_pos));
                }

                if (stream_buffer.size() < static_cast<size_t>(PCIE_FRAME_BYTES)) {
                    break;
                }

                const auto* meta = reinterpret_cast<const PcieFrameMeta*>(stream_buffer.data());
                if (meta->magic != PCIE_MAGIC ||
                    meta->width != static_cast<uint32_t>(width) ||
                    meta->height != static_cast<uint32_t>(height)) {
                    stream_buffer.erase(stream_buffer.begin(),
                                        stream_buffer.begin() + static_cast<std::ptrdiff_t>(sizeof(uint32_t)));
                    continue;
                }

                const uint8_t* pixel_data = stream_buffer.data() + PCIE_META_LEN;
                rgb565_to_rgb888_local(reinterpret_cast<const uint16_t*>(pixel_data),
                                       image_buf_888.data(),
                                       width * height);
                ...
                stream_buffer.erase(stream_buffer.begin(),
                                    stream_buffer.begin() + static_cast<std::ptrdiff_t>(PCIE_FRAME_BYTES));
                if (job_cnt - result_cnt >= numThreads) {
                    break;
                }
            }
