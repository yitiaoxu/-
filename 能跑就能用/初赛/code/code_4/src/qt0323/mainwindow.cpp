...
InferenceThread::InferenceThread(QObject *parent)
    : QThread(parent), keep_running(false), current_input_type(INPUT_CAMERA),
      camera_index(0), yolo11_detector(nullptr), yolo11_thread_pool(nullptr),
      lprnet_ready(false), models_ready(false)
{
    memset(&lprnet_ctx, 0, sizeof(rknn_app_context_t));
}

void InferenceThread::ensureModelsReady() {
    if (models_ready) {
        return;
    }
...
void InferenceThread::run() {
    ...
    if (current_input_type == INPUT_PCIE) {
        emit showMessage("PCIe HDMI 预览 1280x720");
        processPCIeStream();
        emit showMessage("就绪");
        return;
    }
    ...
    if (current_input_type == INPUT_IMAGE) {
        processSingleImage();
    } else {
        processStream();
    }
...
void InferenceThread::processPCIeStream() {
    ...
    constexpr int row_bytes = 2560;

    std::vector<uint8_t> image_buf_565(static_cast<size_t>(width) * height * 2);
    std::vector<uint8_t> image_buf_888(static_cast<size_t>(width) * height * 3);

    DMA_OPERATION dma_operation{};
    dma_operation.current_len = row_bytes / 4;
    ...
    if (!dump_path || !dump_path[0]) {
        dump_path = "/userdata/work/code_4/tools/frame.bin";
    }
    const char* sync_log = std::getenv("PCIE_ROW_SYNC_LOG");
    ...
    while (keep_running) {
        ...
        bool row_ok = true;
        const int64 frame_tick_start = cv::getTickCount();
        for (int row = 0; row < height; ++row) {
            if (sync_log && sync_log[0] == '1' && (row == 0 || row == 360 || row == 719)) {
                std::cout << "[PCIe] row=" << row
                          << " t=" << (cv::getTickCount() - frame_tick_start)
                          << " frame=" << frame_cnt << std::endl;
            }
            memset(dma_operation.data.read_buf, 0, DMA_MAX_PACKET_SIZE);
            if (ioctl(fd, PCI_DMA_WRITE_CMD, &dma_operation) < 0) {
                std::cerr << "[PCIe] PCI_DMA_WRITE_CMD failed row=" << row << std::endl;
                row_ok = false;
                break;
            }
            ...
            if (ioctl(fd, PCI_READ_FROM_KERNEL_CMD, &dma_operation) < 0) {
                std::cerr << "[PCIe] PCI_READ_FROM_KERNEL_CMD failed row=" << row << std::endl;
                row_ok = false;
                break;
            }
            std::memcpy(image_buf_565.data() + static_cast<size_t>(row) * row_bytes,
        ...
        if (!frame_dumped) {
            ...
            if (fp) {
                ...
                const size_t written = std::fwrite(image_buf_565.data(), 1, frame_bytes, fp);
                std::fclose(fp);
                if (written == frame_bytes) {
                    frame_dumped = true;
                    std::cout << "\n[PCIe] 已写出抓帧: " << dump_path
                              << " (" << frame_bytes << " B)\n";
                } else {
                    std::cerr << "[PCIe] 抓帧写入不完整: " << written << "/"
                              << frame_bytes << " @ " << dump_path << std::endl;
                }
            } else {
                std::cerr << "[PCIe] 无法打开抓帧路径: " << dump_path << std::endl;
            }
        }

        rgb565_to_rgb888_local(reinterpret_cast<const uint16_t*>(image_buf_565.data()),
                               image_buf_888.data(),
        ...
        if (frame_cnt % 10 == 0) {
            ...
            double fps = (frame_cnt == 0)
                ? 0.0
                : 10.0 * cv::getTickFrequency() / (fps_end_time - fps_start_time);
            fps_start_time = fps_end_time;
            std::cout << "[PCIe] 预览帧率: " << fps << " FPS (帧 " << frame_cnt << ")\r"
                      << std::flush;
        }
        frame_cnt++;
